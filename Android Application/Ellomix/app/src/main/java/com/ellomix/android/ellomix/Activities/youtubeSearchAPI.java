...
public class youtubeSearchAPI {
    interface AsyncResponse {
        void processFinish(List<Track> outputResult);
    }
    /**
    ...
    public youtubeSearchAPI(AsyncResponse resp, String url) {
        this.callback = resp;
        new BackgroundSearch().execute(url);
    }

    public class BackgroundSearch extends AsyncTask<String, String, List<Track>> {

        @Override
        protected List<Track> doInBackground(String... params) {
            return getYoutubeResults(params[0]);
        }

        @Override
        protected void onPostExecute(List<Track> searchResult) {
            callback.processFinish(searchResult);
        }
    }

    public static List<Track> getYoutubeResults (String query) {
        ...
        try {
            ...
            for (SearchResult video: searchResultList) {
                ...
                Track curTrack = new Track(snippet.getTitle(), snippet.getChannelTitle(), video.getId().getVideoId());
                curTrack.setSource(Sources.YOUTUBE);
                results.add(curTrack);
            }
